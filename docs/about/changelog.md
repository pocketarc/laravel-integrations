...
- `IntegrationCredentialCast::set()` now throws `InvalidArgumentException` when handed an unsupported input type (string, int, object that isn't a `Spatie\LaravelData\Data` instance, etc.) instead of silently returning `null`. Previously, factories that pre-encrypted credentials with `Crypt::encryptString(json_encode(...))` would silently produce rows with `credentials = NULL` that later tripped credential-type guards in SDK clients on first use. Drop the manual encrypt and pass plain arrays — the cast handles encryption.
