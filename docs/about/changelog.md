...
This release adds two columns to the integrations table in a new migration file. The package publishes migrations rather than loading them, so re-run `php artisan vendor:publish --tag=integrations-migrations` before `php artisan migrate`.
