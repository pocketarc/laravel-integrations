...
- Docs: new [Recovering on conflict](/core-concepts/idempotency#recovering-on-conflict) section explaining the catch-and-replay flow, including the redaction caveat — if your provider implements `RedactsRequestData` and strips fields from `response_data`, recovery will replay the redacted version. Don't redact response fields you'd need to recover from.
