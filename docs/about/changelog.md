...
- Recommend per-page cursor checkpointing for adapters with long-running incremental syncs. See [Long-running syncs](/features/scheduled-syncs#long-running-syncs-and-per-page-checkpointing) and the adapter-side [Sync pattern](/adapters/building-adapters#sync-pattern). Companion adapter-side fix landing in `pocketarc/laravel-integrations-adapters`.
