...
One external ID maps to exactly one local row, and the package now enforces that rather than assuming it. See the [upgrade guide](/about/upgrade-guide) for the migration.

- Breaking: [`mapExternalId()`](/features/id-mapping#claiming-and-re-pointing) throws `MappingAlreadyClaimed` when a different model already holds the external ID, instead of silently re-pointing the mapping at the new one. The silent version made a lost race unobservable: two workers upserting the same external ID each inserted a local row, the second took the mapping, and the first was abandoned with every column intact. It kept satisfying ordinary queries, but `findExternalId()` returned null for it and nothing could address it upstream again. One consumer found this as a ticket that was selected for work, attempted, and failed roughly once every two minutes for three days. Calling it twice with the same model is still a no-op.
...
- Fix: [`upsertByExternalId()`](/features/id-mapping#upsert-by-external-id) is serialised per `(integration, model type, external ID)` with a cache lock, and converges on the winner's row if a claim collides anyway. The lock only serialises across processes on a shared cache driver, so the collision path is handled rather than assumed away. TTL and wait are configurable under [`mappings`](/reference/configuration).
- New: [`integrations:find-orphans`](/reference/artisan-commands#integrations-find-orphans) lists rows of a mapped model that have no external ID, for finding damage a pre-6.0 race already did. It compares keys in PHP rather than joining, so a collation mismatch on `internal_id` can't break it.
- New: `integrations.mappings.collation` pins the mapping table's string columns on MySQL and MariaDB. `internal_id` is a VARCHAR holding keys of every type, so comparing it against your own primary keys fails with `Illegal mix of collations` when your tables use a different collation. Null (the default) leaves the connection default. A second migration applies it to existing installs.
