...
3.0 reworks how scheduled syncs advance the cursor: the framework now tracks per-item completion, so the cursor only moves past items whose listeners finished. This closes a silent-data-loss gap where a queued listener that exhausted its retries left the item in `failed_jobs` while the cursor had already moved on.
